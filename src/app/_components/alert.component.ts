...
export class AlertComponent implements OnInit, OnDestroy {
    private scheduleDetectChanges() { setTimeout(() => this.cdr.detectChanges()); }
    ...
    removeAlert(alert: Alert) {
        ...
        if (this.fade) {
            alert.fade = true;
            this.scheduleDetectChanges();
            setTimeout(() => { this.alerts = this.alerts.filter(x => x !== alert); this.scheduleDetectChanges(); }, 250);
        } else {
    ...
    cssClasses(alert: Alert) {
        ...
        if (alert.type !== undefined) { classes.push(alertTypeClass[alert.type]); }
